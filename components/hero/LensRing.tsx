...
export function LensRing() {
  return (
    <div
      data-lens-ring
      aria-hidden="true"
      className="lens-glass pointer-events-none absolute top-0 left-0 z-10 block rounded-full border border-green-signal/70 opacity-0"
      style={{
    ...
    >
      <span className="absolute -inset-[9px] rounded-full border border-green-signal/25" />
      <span className="absolute top-1/2 left-[-1px] h-px w-1.5 -translate-y-1/2 bg-green-signal" />
      <span className="absolute top-1/2 right-[-1px] h-px w-1.5 -translate-y-1/2 bg-green-signal" />
      <span
        data-lens-readout
        className="absolute right-0 -bottom-6 translate-x-1/3 font-mono text-[10px] tracking-wider whitespace-nowrap text-mono-anno"
      />
    </div>
  );
}
