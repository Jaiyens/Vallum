...
- [foreman] Lens wordmark fix: regenerated vine art FROM the rendered typography (12 credits) so glyph geometry matches within 0.05px, alpha-keyed the background, and switched the lens img to eager loading: Chromium never fetches a lazy image inside the masked container, which is why real hovers showed an empty hole even after the art was fixed. 2026-07-19.
- [foreman] Top nav converted to scroll-intent chrome per Jay: fixed bar hidden on load and on downward scroll, slides in on upward scroll, focus-within reveal keeps the first tab stops reachable for keyboard users. 2026-07-19.
